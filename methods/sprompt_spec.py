...
from utils.data_manager_spec import DummyDataset
...
class SPrompts_Spec(BaseLearner):
    ...
    def incremental_train(self, data_manager_spec):
        self._cur_task += 1
        self._total_classes = self._known_classes + data_manager_spec.get_task_size(self._cur_task)
        self._network.update_fc(self._total_classes)
        logging.info('Learning on {}-{}'.format(self._known_classes, self._total_classes))
        # Get Data stored in memory
        if self._cur_task != 0 :
            memory_data, memory_targets = self._get_memory()
        # Constructing the exemplar memory used for rehearsal during the incremental learning setting
        self.build_rehearsal_memory(data_manager_spec, per_class=self.samples_per_class)
        ex_d, ex_t = self._get_exemplar_with_class_idxes(list(range(self._known_classes)))
        train_dataset = data_manager_spec.get_dataset(np.arange(self._known_classes, self._total_classes), source='train',
                                                 mode='train', )
        ...
        test_dataset = data_manager_spec.get_dataset(np.arange(0, self._total_classes), source='test', mode='test')
